...
/* etap 1 : Enumeration des cartes avec leur nombre de points. */
...
int main ()
{
  ...
  /*  etape 2 :Initialisation du generateur de nombres aleatoires.*/
  ...
  /*verification que la reponse est bien 'y' si non on repose la question. */
  ...
  /* chaque fois que le joueur demande une carte, le jeu continue. */
  ...
  while (jouer=='y') {
    ...
    /* Affecter � la variable C1 le poid de la carte tiree suivant sa valeur d'apres
    ...
    /* verification du score du joueur, s'il est <= 21 le jeu continue sinon 
       le jeu est termine (partie perdue). */
  ...
  if (jouer == 'n') {
    ...
    /* Calcul du score de la banque (sachant que le score de la banque est un tirage aleatoire entre 16 et 21).*/
    score_banque=16+rand()%6;
    
    printf("Le score de la banque est de  %d \n", score_banque);
