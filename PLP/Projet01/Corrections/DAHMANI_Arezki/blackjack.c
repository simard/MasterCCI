...
int main(int argc, char *argv[]) {
    ...
    char decisionJoeur; // choix deu joueur
    int scoreJoeur, scoreBanque; // score du joeur et de la banque
    int tirageCarte, tirageCarteBanque; // carte tirees
    int nbAs = 0; // nombre d'As tires
    enum carteTiree {C0,C1,C2,C3,C4,C5,C6,C7,C8,C9,CX,CVDR, // avec C := carte coeur
        T1,T2,T3,T4,T5,T6,T7,T8,T9,TX,TVDR, // avec T := carte trefle
        P1,P2,P3,P4,P5,P6,P7,P8,P9,PX,PVDR, // avec P := carte Pique
        CA1,CA2,CA3,CA4,CA5,CA6,CA7,CA8,CA9,CAX,CAVDR}; // avec CA : carte Carreau
        // X=10, VDR=ValetDameRoi
	...
	decisionJoeur = '0';
        ...
        // tant que le score du joeur < 21 , on continue sauf s'il decide d'arreter
	do {
		printf("Votre score est : %d\n", scoreJoeur );
		while (decisionJoeur != 'y' && decisionJoeur != 'n'){
                    //tant que caractère entre est different de y et n , on lui demande de prendre une decision
			...
			printf("\n");
                ...
                if (decisionJoeur=='n')
                    break; // si le joeur decide d'arreter de jouer, on sort de la boucle
                if (decisionJoeur=='y') // sinon on lui sert une carte
                    tirageCarte = (int) (1 + ((double) rand() / RAND_MAX * (52 - 1 + 1)));
                // on prend une valeur aleatoire entre 1 et 52
                // switch traite les carte tirée :  type et valeur equivalente
                switch (tirageCarte) {
                ...
                if (scoreJoeur>21 && nbAs>0) { // si le score depase 21 et il a un as
                    scoreJoeur -= 10; // donc on retire 10
                    nbAs--; //et on diminue le nombre d'As tire
                }
                if (scoreJoeur>21 && nbAs==0) {  // si le score depase 21 et il n'a pas un As
                    printf("Vous avez perdu \n"); // le joeur perds la partie
                    break;
                }
                decisionJoeur = '0';
        ...
        if (scoreJoeur >21) { // la banque peut tirer n'iport quelle carte puisque la joeur a deja perdu
        ...
        if (scoreJoeur == 0) { // si que le joeur ne voulais jouer, il faut donc l'inciter à jouer !!!
            printf("Trouvez de l'aregent et venez jouez !!! \n");
            printf("Tentez votre chance !!! \n");
        }

        if (scoreJoeur >= 1 && scoreJoeur <= 21) { // le joeur s'est arrete de jouer avec un score < 21
            // donc c'est le tour de la banque
            // on tire un entier entre 16 et 21
            scoreBanque = (int) (16 + ((double) rand() / RAND_MAX * (21 - 16 + 1)));
            if (scoreBanque <= scoreJoeur) { // le joeur gagne s'il a un score superier ou égale au score de la Banque
                printf("Score Banque est   : \t %d \n", scoreBanque);
                printf("Fin de la partie, votre score est : %d \t, vous avez gagne !!! \n", scoreJoeur );
            }
            else { // le joeur perd s'il a un score strictement inferieur au score de la Banque
                printf("Score Banque est   : \t %d \n", scoreBanque);
                printf("Fin de la partie, votre score est : %d \t, vous avez perdu !!! \n", scoreJoeur );
            }
        }
