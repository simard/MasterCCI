...
/* Declaration de la bibliotheque du travail */
...
/* Definition de l'environnement pour la fonction system() */
...
{
	/*declaration des variables globales du projet : dimension cadre, nombres de serpents et temps d affichage */
	int ilargeur,ihauteur,nbserpents,temps_attente;

	/* recuperation des arguments de la commande */
	/* teste si le nombre d'arguments est egale a celui specifier pour snake */
	if (argc !=4)
	{
	...
	for(k=0;k<nbserpents;k++)
	{
		...
		printf("%d\n",stab[k].xposition);
		printf("%d\n",stab[k].yposition);
		cEchiquier[stab[k].xposition*ilargeur+stab[k].yposition]=stab[k].sens;  /* Placement du serpent dans le cadre d affichage */
	}

	/* affichage de l'envirennement*/
	init_environnement( ihauteur, ilargeur,cEchiquier);

	/* teste si tous les serpents sont bloquees ou pas */
	while(somme_blocage != nbserpents)
	{
		/* boucle qui gerent les deplacements des serpents */
		for(k=0;k<nbserpents;k++)
		{
			switch(stab[k].sens)
			{
	...
	/* Partie statisques du programme */
	...
	return 0;
} /* fin du principale du programme */


/* Definition de la fonction d initialisation de l' environnement */
...
/* definition de la fonction d initialisation de tableau */
