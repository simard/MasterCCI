...
/* la ceation d'une structure qui contien les trois chaine de caract√©re : le nom, le prenom et l'adresse courriel*/
...
/* Fonction ajouter contact au debut ou a la fin de la chaine*/
...
/*construction de la Fonction qui affiche un seul contact*/
...
if(Tete==NULL)
	...
	printf("Nombre de contacts affichÈrs: %d \n",i);
	}
}
...
else
{
	...
	while(p!= NULL)
	{	
        /*l'√©criture de contenu de p dans le fichier pFichier*/
		fwrite(p, sizeof(Contact),1, pFichier);
		p=p->suivant;	
	}
	fclose ( pFichier); /*la Fermeture du fichier a la fin*/
	printf("l'Ècriture dans le fichier %s\n",NomFichier);
}
}

/*la Licture √† partir d'un fichier*/
int ChargerListe()
{
FILE * pFichier = NULL ;
...
if(pFichier==NULL) 
{
	...
	while( fread(p, sizeof(Contact), 1, pFichier)!=0)
    {
		...
		printf("l'Èlement ajouter est %d",i);
		i++;
	}
	/*dans les deux cas (1 et 2) le dernier espace a √©t√© allou√©our rien*/
	free(p); 
	/*Fermer le fichier vers la fin*/
	fclose ( pFichier); 
	printf("L'Ècture† ‡ partir du fichier %s\n",NomFichier);
}
}
...
/*le traitement d'abord de la tete*/
while (Tete!=NULL && strcmp(Nom,Tete->cNom)==0)
{
			printf("Contact Prenom: %s, Nom: %s, Courriel: %s trouv√©. Voulez vous le supprimer? Taper o si oui ou sinon taper autre touche \n", Tete->cPrenom,Tete->cNom,Tete->cCourriel);
			scanf("%s",cchaine);
			cc=cchaine[0];
...
while( Courant !=NULL) 
{
	...
	if(strcmp(Nom,Courant->cNom)==0)
    {
        ...
        { 
			...
			/*la Liberation de l'espace occupÈ*/
			free(Vide); 
			NbSupp++;
			}else{ 
			Precedent=Courant;
...
while(Courant !=NULL)
{
/* pour trouv√© la chaine */
	if(strcmp(Nom,Courant->cNom)==0)
    { 
		printf("Contact Prenom: %s, Nom: %s, Courriel: %s trouv√©. Voulez vous le modifier? Taper o si oui ou  sinon autre touche\n", Courant->cPrenom,Courant->cNom,Courant->cCourriel);
		scanf("%s",cchaine);
		cc=cchaine[0];
...
int main ( int argc, char ** argv )
{
	...
	{	
		...
		printf ( "6 - le Chargement d'un carnet d'addresses ‡† partir d'un fichier\n") ;
		printf ( "7 - la Liberation de l'espace mÈmoire.\n") ;
		printf ( "8 - Quitter\n") ;
        scanf("%s",choixx);
		...
		case '1' :
		{
			...
			if (r==1) printf("Le contact %s  %s ( E-mail: %s ) a √©t√© ajoout√© \n", C->cPrenom,C->cNom,C->cCourriel);		
		...
		case '3' :
			...
			if(Tete!=NULL)
			{
                ...
                printf("le Nombre de contacts a √©t√© modifie: %d",r);
			}else{
				printf("Le carnet d'addresses est vide!!");
			}
			break ;
		...
		case '4' :
			...
			if(Tete!=NULL)
			{
				printf ("donner le Nom du contact a supprim√©\n") ;
				scanf ("%s", Nom) ; 
				int r=SupprimerContact(Nom);
				if(r==0) 
                printf("le contact est introuvable dans le carnet d'addresse\n");
				else 
                printf("Nombre de contacts a √©t√© supprim√©s: %d",r);
			}else{
				printf("Le carnet d'addresses est vide!!");
			}
			break ;
...
/*le Chargement du carnet de contacts a partir d'un fichier*/
		case '6' :
			{
      /* pour la Liberation de l'espace memoire occup√© par l'ancienne liste on fait */ 
   			ViderListe(); 
			ChargerListe();
			break ;	
		} 
/*le Chargement du carnet de contacts a partir d'un fichier*/
		case '7' :
			{ 
/*la Liberation de l'espace memoire occup√© par l'ancienne liste ce fait comme suite*/
			ViderListe(); 
			break ;	
		} 
/*pour Quitter le programme*/
		case '8' :
			...
			printf ("la Lib√©ration de l'espace m√©moire\n") ;
			ViderListe();
			break ;	
		} 
